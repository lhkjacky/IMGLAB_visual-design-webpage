# Transcript → WordPress Block Handoff v2.1
...
## ⚡ WHAT'S NEW IN v2.1 — READ BEFORE STARTING

Three changes from v2.0. Read all three before writing any HTML.

### Change 1 — `--cv-tinted-bg` is now a required 12th palette variable

The global stylesheet (`woodmart-child/style.css`) now uses `var(--cv-tinted-bg, #f8fafc)` to set the `.cv-tinted` section background. Every post that uses `.cv-tinted` **must** define this variable with a very light color matching the article palette. The fallback `#f8fafc` is functional but neutral — always override it with something palette-matched.

**Choosing `--cv-tinted-bg`:** Pick the lightest possible tint of the article's accent color. Dark text (`#0f172a`, `#334155`, `#475569`) must be clearly readable on top of it.

Examples by accent family:
- Cyan/teal accent → `#e0f9f9` or `#f0fdfa`
- Orange accent → `#fff7ed` or `#fef3e2`
- Purple accent → `#faf5ff` or `#f3e8ff`
- Green accent → `#f0fdf4` or `#ecfdf5`
- Blue accent → `#eff6ff` or `#dbeafe`
- Red accent → `#fff5f5` or `#fef2f2`

Never use a dark or mid-tone color for `--cv-tinted-bg`. If in doubt use the Tailwind 50 or 100 level equivalent.

Add it as the 12th variable in the per-post palette block:
```css
.content-visual {
  /* ... existing 11 variables ... */
  --cv-tinted-bg: [very light accent tint];
}
```

### Change 2 — Never use inline `style="color:..."` on text elements

**This is the root cause of all `.cv-tinted` dark mode failures investigated in v2.1.**

The WP Dark Mode plugin injects rules like:
```css
html.wp-dark-mode-active p { color: #ccc !important; }
```

When an element already has an inline `style="color:#475569"`, the plugin's `!important` wins over the inline style. Any `[data-wp-dark-mode-ignore]` rule in a stylesheet — even with `!important` — **cannot override an inline style that has already been overridden by another `!important`**. This is a CSS specificity dead-end with no stylesheet solution.

**The fix is architectural:** never put `color` values in inline styles on text elements. Always use a CSS class. The `[data-wp-dark-mode-ignore]` ancestor rules in the global stylesheet target class names with higher specificity than the plugin's generic element-level selectors, and they reliably win.

```html
✅ CORRECT — color from class, overrideable by global stylesheet:
<p class="cv-body wp-dark-mode-ignore">Text</p>
<strong class="cv-tinted-step-title wp-dark-mode-ignore">Title</strong>

❌ WRONG — inline color, plugin overrides with !important, no stylesheet fix possible:
<p style="color:#475569;">Text</p>
<strong style="color:#0f172a;">Title</strong>
```

**Allowed inline styles:** `background`, `border-color`, `padding`, `margin`, `gap`, `font-size`, `font-weight`, `display`, flex/grid layout properties. The plugin does not aggressively override these.

**Never inline:** `color`. Always use a class.

This rule applies everywhere in the HTML — not just inside `.cv-tinted`. Any element with `style="color:..."` is a potential dark mode failure on any future plugin update.

### Change 3 — `cv-tinted-step` is now a named class set in the global stylesheet

When building numbered process steps inside `.cv-tinted`, use the class-based component instead of anonymous divs with inline styles. This makes dark mode overrides reliable (following Change 2 above).

**New global classes (already in `woodmart-child/style.css`):**
- `.cv-tinted-step` — card container with white semi-transparent background
- `.cv-tinted-step-hd` — flex row containing number + title
- `.cv-tinted-step-num` — numbered circle (uses `var(--cv-accent)` as background automatically)
- `.cv-tinted-step-title` — heading text
- `.cv-tinted-step-body` — paragraph text

**HTML pattern:**
```html
<div class="cv-tinted cv-mb wp-dark-mode-ignore">
  <div class="cv-card-title wp-dark-mode-ignore">
    <i class="fa-solid fa-[icon] wp-dark-mode-ignore" style="color:var(--cv-accent);"></i>
    Section Title
  </div>
  <p class="cv-body wp-dark-mode-ignore">Optional intro paragraph.</p>
  <div class="cv-grid-2 wp-dark-mode-ignore" style="gap:0.75rem;">
    <div class="cv-tinted-step wp-dark-mode-ignore">
      <div class="cv-tinted-step-hd wp-dark-mode-ignore">
        <div class="cv-tinted-step-num wp-dark-mode-ignore">1</div>
        <strong class="cv-tinted-step-title wp-dark-mode-ignore">Step Title</strong>
      </div>
      <p class="cv-tinted-step-body wp-dark-mode-ignore">Step description.</p>
    </div>
...
Note: `cv-tinted-step-num` picks up `var(--cv-accent)` automatically from the global stylesheet — no inline `background` needed.
...
- All `cv-tinted-step` component styles: `.cv-tinted-step`, `.cv-tinted-step-hd`, `.cv-tinted-step-num`, `.cv-tinted-step-title`, `.cv-tinted-step-body`
...
1. **CSS variables** — the 12 per-article palette values (see Step 8 for full list including `--cv-tinted-bg`)
...
.content-visual {
  ...
  --cv-tinted-bg:       [very light accent tint — dark text must be readable on it];
}
...
>
> **Mistake D — inline `style="color:..."` used on text elements inside `.cv-tinted`, causing dark mode failures that no stylesheet rule could fix.**
> The root cause: when WP Dark Mode sets `color: #ccc !important` on an element via its own injected CSS, and that element also has an inline `style="color:#475569"`, the plugin's `!important` wins — because `!important` inline styles and `!important` stylesheet rules are resolved by source order, with the plugin's rule loading last. No `[data-wp-dark-mode-ignore]` selector in any stylesheet can override this. The only solution is architectural: **never use inline `color:` on any text element**. Always assign text colors via CSS classes so the global stylesheet's `[data-wp-dark-mode-ignore]` ancestor rules can reliably override the plugin. See v2.1 Change 2 for the full explanation and correct pattern.
...
[data-wp-dark-mode-ignore] .cv-tinted           { background: var(--cv-tinted-bg, #f8fafc) !important; }
[data-wp-dark-mode-ignore] .cv-tinted .cv-card-title { color: #0f172a !important; }
[data-wp-dark-mode-ignore] .cv-tinted .cv-body  { color: #475569 !important; }
[data-wp-dark-mode-ignore] .cv-tinted > p       { color: #334155 !important; }
[data-wp-dark-mode-ignore] .cv-tinted strong    { color: #0f172a !important; }
[data-wp-dark-mode-ignore] .cv-tinted-step      { background: rgba(255,255,255,0.82) !important; }
[data-wp-dark-mode-ignore] .cv-tinted-step-title { color: #0f172a !important; }
[data-wp-dark-mode-ignore] .cv-tinted-step-body { color: #475569 !important; }
...
### `--cv-tinted-bg` — Tinted Section Background
The background color for `.cv-tinted` sections. Must be a very light tint of the accent color — light enough that dark text (`#0f172a`, `#334155`, `#475569`) is clearly readable on top. Think of it as the palest possible wash of the palette — never a mid-tone, never dark.

Pick from the lightest end of your accent hue (Tailwind 50 or 100 level equivalent). If unsure, test the contrast: the color you choose must pass WCAG AA (4.5:1) against `#334155`. This variable is set per-post and used by the global stylesheet's dark mode protection rule.
...
### Tinted Section (with named step cards)
Use `.cv-tinted-step` classes — not anonymous divs with inline colors — so dark mode overrides work reliably. See v2.1 Change 3 for explanation.
```html
<div class="cv-tinted cv-mb wp-dark-mode-ignore">
  <div class="cv-card-title wp-dark-mode-ignore">
    <i class="fa-solid fa-[icon] wp-dark-mode-ignore" style="color:var(--cv-accent);"></i>
  ...
  <div class="cv-grid-2 wp-dark-mode-ignore" style="gap:0.75rem;">
    <div class="cv-tinted-step wp-dark-mode-ignore">
      ...
      <p class="cv-tinted-step-body wp-dark-mode-ignore">Step description text.</p>
    </div>
    <!-- repeat for each step -->
  </div>
</div>
...
## 📐 STEP 8 — Per-Post `<style>` Block (v2.1)
...
.content-visual {
  ...
  --cv-cc-featured-bg:  [accent tint for featured competitor card];
  --cv-table-head-bg:   [dark color for table headers, usually #0f172a];
  --cv-tinted-bg:       [very light accent tint for .cv-tinted sections — dark text must be readable on it];
}
...
/* cv-tinted background: global sheet uses var(--cv-tinted-bg) — defined above in palette */
/* No separate override line needed here — the variable is picked up automatically */
...
### R1a — Never Use Inline `style="color:..."` on Text Elements
**This is a dark mode architectural constraint, not a preference.** The WP Dark Mode plugin overrides inline color styles with `!important`. Once the plugin sets a color inline, no stylesheet rule — not even `[data-wp-dark-mode-ignore]` with `!important` — can change it back. The only reliable protection is class-based colors, because the global stylesheet's `[data-wp-dark-mode-ignore]` ancestor rules target class names with higher specificity than the plugin's generic element selectors.

Always use a CSS class for text color. Inline `background`, `padding`, `margin`, `font-size`, `font-weight`, and layout properties are safe and may be used inline as before.
...
2. HTML comment (e.g. `<!-- Article Title | WordPress Block -->`) — no `<link>` tag; Font Awesome is global
...
- [ ] **No inline `style="color:..."` on any text element** — all text colors set via CSS classes so dark mode overrides work (v2.1 R1a)
- [ ] **`--cv-tinted-bg` variable set in post palette** (required if `.cv-tinted` is used in this post)
...
- [ ] `--cv-tinted-bg` is a very light color — dark text clearly readable on it (Tailwind 50/100 level)
- [ ] All text on non-white backgrounds has explicit color set via CSS class (not inline style)
...
*End of handoff v2.1. Provide this file + a YouTube transcript (any language) → translate, verify all specs and terminology, convert all units correctly, find and verify images, and generate **both deliverables** — the complete Japanese WordPress HTML block AND the SEO post title text file — immediately. The `<div>` must appear before `<style>` in the HTML file. Never use inline `style="color:..."` on text elements. Set `--cv-tinted-bg` in the palette. Use your full capabilities. Take the time needed. Prioritize quality above all else. Make it excellent.*